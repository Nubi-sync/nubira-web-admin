...
import { PrintingWorker, PrintingWorkerRole } from '../types/printing'
import { deletePrintingWorker, savePrintingWorker } from '../utils/printingFloorStorage'
import { ConfirmDialog } from '@/components/ui/ConfirmDialog'
import { deletePrintingWorkerAction } from '../actions'
...
export function WorkerListModal({
  ...
  const [workerToDelete, setWorkerToDelete] = useState<PrintingWorker | null>(null)
  ...
  const filteredWorkers = workers.filter(w => {
    ...
    const matchesRole = 
      roleFilter === 'ALL' || 
      (w.roles && w.roles.includes(roleFilter as PrintingWorkerRole)) ||
  ...
  const handleConfirmDeleteWorker = async () => {
    ...
    try {
      ...
      deletePrintingWorker(workerToDelete.id)
      if (workerToDelete.worker_name) deletePrintingWorker(workerToDelete.worker_name)
      if (workerToDelete.phone_number) deletePrintingWorker(workerToDelete.phone_number)

      if (onDeleteWorker) {
        await onDeleteWorker(workerToDelete.id, workerToDelete.phone_number)
      } else {
        await deletePrintingWorkerAction(workerToDelete.id, workerToDelete.phone_number)
      }

      toast.success(`Worker "${workerToDelete.worker_name}" removed from printing roster.`)
      onWorkersUpdated()
    } catch (err) {
  ...
  const handleToggleStatus = (worker: PrintingWorker) => {
    const nextStatus: PrintingWorker['status'] = worker.status === 'ACTIVE' ? 'ON_LEAVE' : 'ACTIVE'
    savePrintingWorker({ ...worker, status: nextStatus })
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-150 select-none">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-mono">
                Manage registered print operators, technique specializations, and portal credentials
              </p>
            </div>
          </div>
        ...
        <div className="p-4 border-b border-black/10 bg-slate-50/70 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
            {[
              ...
              { id: 'SCREEN_PRINTER', label: 'Screen Print' },
              { id: 'DTG_TECHNICIAN', label: 'DTG Tech' },
              { id: 'TABLE_OPERATOR', label: 'Table Master' }
            ].map(r => (
              <button
                key={r.id}
                type="button"
        ...
        <div className="p-6 overflow-y-auto space-y-3">
          {filteredWorkers.length === 0 ? (
            <div className="py-12 text-center text-slate-400 space-y-2">
              ...
              <p className="text-sm font-semibold">No registered printing workers found.</p>
              <button
                type="button"
                onClick={() => {
                  onClose()
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {filteredWorkers.map(worker => {
                ...
                const displayRoles = worker.roles && worker.roles.length > 0
                  ...
                  : (worker.role ? [worker.role] : ['Screen Print Operator'])
      ...
      <ConfirmDialog
        ...
        title="Remove Printing Worker"
        description={`Are you sure you want to remove "${workerToDelete?.worker_name}" from the Printing Floor roster? They will no longer be available for shift task allocations.`}
            ...
            <div className="flex items-center justify-between border-t border-black/5 pt-2">
              ...
              <span className="font-bold text-slate-800">
                {(workerToDelete.roles && workerToDelete.roles.length > 0)
                  ...
                  : (workerToDelete.role || 'Screen Print Operator')}
              </span>
            </div>
          </div>
        )}
