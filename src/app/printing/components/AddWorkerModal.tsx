...
import { savePrintingWorker } from '../utils/printingFloorStorage'
import { registerPrintingWorkerAction } from '../actions'

interface AddWorkerModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess?: (worker: PrintingWorker) => void
}

const AVAILABLE_ROLES: { id: PrintingWorkerRole; label: string }[] = [
...
export function AddWorkerModal({ isOpen, onClose, onSuccess }: AddWorkerModalProps) {
  ...
  const [selectedRoles, setSelectedRoles] = useState<PrintingWorkerRole[]>(['SCREEN_PRINTER'])
  ...
  const toggleRole = (roleId: PrintingWorkerRole) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newWorker: PrintingWorker = {
        id: `pw-${Date.now()}`,
      ...
      savePrintingWorker(newWorker)

      // Provision Auth User & database row via server action
      const res = await registerPrintingWorkerAction({
        worker_name: name.trim(),
        phone_number: phone.trim(),
      ...
      setPhone('')
      setPassword('')
      setSelectedRoles(['SCREEN_PRINTER'])
      onClose()
    } catch (err: any) {
