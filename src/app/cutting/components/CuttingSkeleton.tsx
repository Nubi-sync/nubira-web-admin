...
export function SkeletonTableGrid({ count = 4 }: { count?: number }) {
  return (
...
export function SkeletonCardGrid({ count = 4 }: { count?: number }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-white p-5 rounded-2xl border border-black/10 shadow-2xs space-y-4">
...
export function CuttingPageSkeleton({
  variant = 'dashboard'
}: {
  variant?:
    | 'dashboard'
