...
export function WorkerHistoryClient({
  ...
  const filtered = completedTasks.filter(t =>
    ...
    (t.source_department && t.source_department.toLowerCase().includes(search.toLowerCase()))
  )
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 sm:p-6 rounded-2xl border border-black/10 shadow-2xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 font-medium">
              Verified finished lots and production log for {userName || 'Tailor'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="px-3.5 py-2 rounded-xl bg-[#FAF7F0] border border-black/10 text-xs">
            <span className="text-slate-500 font-medium">Total Assembled: </span>
            <span className="font-bold text-[#3A3564] font-mono text-sm">{totalFinishedPieces.toLocaleString('en-IN')} pcs</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-black/10 shadow-2xs">
          <div className="text-xs text-slate-500 font-medium">Completed Batches</div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-black/10 shadow-2xs">
          ...
          <div className="text-2xl sm:text-3xl font-bold font-[family-name:var(--font-heading)] text-slate-900 mt-2 font-mono">
            {totalFinishedPieces.toLocaleString('en-IN')} pcs
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-black/10 shadow-2xs overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between gap-3">
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              type="text"
              placeholder="Search lot #, article, source..."
              value={search}
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-12 text-center text-slate-400">
                    ...
                    <p className="text-xs font-medium">No completed sewing lots found</p>
                  </td>
                </tr>
              ) : (
                filtered.map(t => (
                  <tr key={t.id} className="hover:bg-slate-50/70 transition-colors">
                    <td className="py-3 px-4 font-mono font-bold text-slate-900">
                      {t.lot_number}
                      <span className="block text-[10px] text-slate-400 font-mono">{t.task_ref}</span>
                    </td>
                    <td className="py-3 px-4 font-bold text-slate-900">
                      {t.article_name}
                      {t.buyer_name && <span className="block text-[10px] text-slate-400 font-normal">{t.buyer_name}</span>}
                    </td>
