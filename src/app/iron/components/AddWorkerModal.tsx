...
import { saveIronWorker } from '../utils/ironFloorStorage'
import { registerIronWorkerAction } from '../actions'

interface AddWorkerModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess?: (worker: IronWorker) => void
  companyName?: string
}

const AVAILABLE_ROLES: { id: IronWorkerRole; label: string }[] = [
...
export function AddWorkerModal({ isOpen, onClose, onSuccess, companyName }: AddWorkerModalProps) {
  ...
  const [selectedRoles, setSelectedRoles] = useState<IronWorkerRole[]>(['FINISHING_PRESSER'])
  const [assignedTable, setAssignedTable] = useState('Table 01 (Vacuum Buck)')
  ...
  const toggleRole = (roleId: IronWorkerRole) => {
    if (selectedRoles.includes(roleId)) {
      if (selectedRoles.length === 1) {
        toast.info('Presser must have at least one floor role.')
        return
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) {
      toast.error('Please enter presser full name.')
      return
    }
    ...
    try {
      ...
      const newWorker: IronWorker = {
        id: `iw-${Date.now()}`,
        worker_name: name.trim(),
        phone_number: phone.trim(),
        roles: selectedRoles,
        role: primaryRoleLabel as any,
        is_active: true,
      ...
      saveIronWorker(newWorker)

      // Provision Auth User & database row via server action
      const res = await registerIronWorkerAction({
        worker_name: name.trim(),
        phone_number: phone.trim(),
      ...
      if (res.success && res.worker) {
        toast.success(`Iron Presser "${name}" registered successfully with phone +91 ${phone}!`)
        onSuccess?.(res.worker)
      } else {
        toast.success(`Iron Presser "${name}" saved to floor roster!`)
        onSuccess?.(newWorker)
      }

      onClose()
    } catch (err: any) {
      console.error('Error saving presser:', err)
      toast.error(err.message || 'Failed to save presser.')
    } finally {
        ...
        <div className="bg-[#FAF7F0] border-b border-black/10 p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-white border border-black/10 flex items-center justify-center text-[#3A3564] shadow-2xs">
              <UserPlus className="w-5 h-5 text-[#3A3564]" />
            </div>
            <div>
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-4">
          ...
          <div>
            <label className="block text-xs font-mono font-bold uppercase tracking-wider text-slate-500 mb-1">
              Presser Full Name <span className="text-rose-500">*</span>
            </label>
            <input
              type="text"
              required
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1">
              Used as presser phone login credential
            </p>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder="Min 6 characters (e.g. iron@123)"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 text-xs rounded-xl border border-black/10 bg-slate-50 focus:bg-white focus:outline-hidden font-semibold"
              >
                <option value="SHIFT_1">Shift 1 (08:00 AM - 04:00 PM)</option>
                <option value="SHIFT_2">Shift 2 (04:00 PM - 12:00 AM)</option>
                <option value="GENERAL">General (09:00 AM - 06:00 PM)</option>
              </select>
            </div>
          </div>
