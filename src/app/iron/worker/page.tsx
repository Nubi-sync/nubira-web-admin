...
import { fetchIronTaskAllocationsAction, fetchIronWorkersAction } from '../actions'

export const dynamic = 'force-dynamic'

export const metadata = {
  title: 'Worker Portal | Steam Ironing Floor',
  description: 'Floor presser workstation for assigned pressing jobs, vacuum table runs, and completion sign-offs.'
}
...
export default async function IronWorkerPage({
  ...
  if (params?.tab === 'history') {
    redirect('/iron/worker/history')
  }
  if (params?.tab === 'profile') {
    redirect('/iron/worker/profile')
  }
  ...
  if (!displayName || displayName === 'Floor Operator' || displayName === 'Iron Operator' || displayName === 'Finishing Presser' || displayName === 'Iron Floor Operator') {
    const metaName = user.user_metadata?.full_name
    if (metaName && metaName !== 'Floor Operator' && metaName !== 'Iron Operator' && metaName !== 'Finishing Presser' && metaName !== 'Iron Floor Operator') {
  ...
  return (
    <AdminShell userEmail={tenant.userEmail} userRole={tenant.role || 'IRON_WORKER'}>
