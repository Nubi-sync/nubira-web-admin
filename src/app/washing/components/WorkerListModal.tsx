...
import { deleteWashingWorker, saveWashingWorker } from '../utils/washingFloorStorage'
import { ConfirmDialog } from '@/components/ui/ConfirmDialog'
import { deleteWashingWorkerAction } from '../actions'
...
const ROLE_LABELS: Record<string, string> = {
  WASH_MASTER: 'Washing Master / Head Chemist',
  HYDRO_EXTRACTOR: 'Hydro Extraction Operator',
  TUMBLER_OPERATOR: 'Industrial Tumbler Dryer Operator',
  CHEMICAL_MIXER: 'Chemical Dosing Specialist',
  SHRINKAGE_INSPECTOR: 'Shrinkage & Shade QC Inspector',
  FINISHING_LOADER: 'Wet Goods Conveyor & Loader'
}
...
export function WorkerListModal({
  ...
  const [workerToDelete, setWorkerToDelete] = useState<WashingWorker | null>(null)
  ...
  const filteredWorkers = workers.filter(w => {
    ...
    const matchesRole = 
      roleFilter === 'ALL' || 
      (w.roles && w.roles.includes(roleFilter as WashingWorkerRole)) ||
  ...
  const handleConfirmDeleteWorker = async () => {
    ...
    try {
      ...
      deleteWashingWorker(workerToDelete.id)
      if (workerToDelete.worker_name) deleteWashingWorker(workerToDelete.worker_name)
      if (workerToDelete.phone_number) deleteWashingWorker(workerToDelete.phone_number)

      if (onDeleteWorker) {
        await onDeleteWorker(workerToDelete.id)
      } else {
        await deleteWashingWorkerAction(workerToDelete.id)
      }

      toast.success(`Worker "${workerToDelete.worker_name}" removed from washing roster.`)
      onWorkersUpdated()
    } catch (err) {
  ...
  const handleToggleStatus = (worker: WashingWorker) => {
    const nextStatus: WashingWorker['status'] = worker.status === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE'
    saveWashingWorker({ ...worker, status: nextStatus })
    toast.info(`${worker.worker_name} marked as ${nextStatus}`)
    onWorkersUpdated()
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-150 select-none">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900 font-[family-name:var(--font-heading)]">
                Washing Floor Operators &amp; Roster
              </h3>
              <p className="text-xs text-slate-500 font-mono">
                {workers.length} registered washers and tumble drying operators
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-4 border-b border-black/5 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
            ...
            >
              ...
              <option value="HYDRO_EXTRACTOR">Hydro Extractors</option>
              <option value="TUMBLER_OPERATOR">Tumbler Operators</option>
              <option value="CHEMICAL_MIXER">Chemical Mixers</option>
            </select>
        ...
        <div className="p-4 overflow-y-auto flex-1 divide-y divide-black/5">
          {filteredWorkers.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <div className="text-sm font-bold text-slate-700">No washing operators found</div>
              <p className="text-xs text-slate-400 mt-1 max-w-xs mx-auto">
                {workers.length === 0 
                  ? 'No operators registered for this company yet. Click "+ Add Worker" to enroll staff.' 
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-slate-500 font-mono">
                      ...
                      {worker.assigned_machine && (
                        <span>• Machine: {worker.assigned_machine}</span>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {(worker.roles || [worker.role]).map((r, idx) => (
      ...
      <ConfirmDialog
        ...
        confirmText={isDeletingWorker ? 'Removing...' : 'Remove Worker'}
        cancelText="Cancel"
        variant="danger"
