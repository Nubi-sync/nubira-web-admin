...
export function WorkerProfileClient({
  ...
  const rolesList: string[] = workerRecord?.roles || (workerRecord?.role ? [workerRecord.role] : ['WASHING_OPERATOR'])
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900 font-[family-name:var(--font-heading)]">
                {userName || 'Washing Floor Operator'}
              </h1>
            ...
            <p className="text-sm text-slate-600 mt-1 font-mono">
              Floor ID: {workerRecord?.id || userId ? `OP-${(workerRecord?.id || userId).slice(-6)}` : 'OP-WASH-01'} • Nubira Creation Floor
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-black/10 shadow-2xs space-y-4">
          <div className="flex items-center gap-2.5 text-[#3A3564] font-bold text-sm">
            <Lock className="w-4.5 h-4.5 text-[#3A3564]" />
          ...
          <div className="space-y-3 font-mono text-xs">
            <div className="flex items-center justify-between p-3 rounded-xl bg-[#FAF7F0]/60 border border-black/5">
              ...
              <span className="font-bold text-slate-900">{userName || 'Washing Operator'}</span>
            </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-black/10 shadow-2xs space-y-4">
          <div className="flex items-center gap-2.5 text-[#3A3564] font-bold text-sm">
            <Waves className="w-4.5 h-4.5 text-[#3A3564]" />
            <span>Assigned Floor Roles</span>
          </div>

          <p className="text-xs text-slate-500 font-mono">
            Operational roles assigned to this operator by the Washing Master &amp; Floor Supervisor
          </p>
          ...
          <div className="p-3.5 rounded-xl bg-slate-50 border border-black/5 text-xs text-slate-600 font-mono space-y-1 mt-4">
            ...
            <div>• Receive article wash piece quotas &amp; drum machine assignments</div>
            <div>• Start live liquor cycle countdown &amp; record chemical dosages</div>
            <div>• Submit finished washed batches to Head of Dept for verification</div>
          </div>
        </div>
