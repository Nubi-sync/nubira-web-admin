...
import { fetchWashingTaskAllocationsAction } from '../../../actions'
...
export function WorkerHistoryClient({
  ...
  const [tasks, setTasks] = useState<WashingTaskAllocation[]>([])
  ...
  const handleManualSync = async () => {
    ...
    try {
      ...
      const serverTasks = await fetchWashingTaskAllocationsAction(companyFilter)
  ...
  let displayWorkerName = userName || 'Washing Operator'
  const isGeneric = (n?: string) => !n || ['floor operator', 'washing operator', 'washing floor operator'].includes(n.trim().toLowerCase())
  ...
  if (isGeneric(userName)) {
    ...
    if (foundFromTask?.worker_name) {
      ...
      if (!foundWorker) {
        try {
          const localWorkers = getWashingWorkers(companyName)
  ...
  // Completed history: VERIFIED_COMPLETED or WORKER_COMPLETED
  const historyTasks = effectiveTasks.filter(t => t.status === 'VERIFIED_COMPLETED' || t.status === 'WORKER_COMPLETED')
  ...
  const filteredHistory = historyTasks.filter(t => {
    ...
    return (
      ...
      (t.machine_station || '').toLowerCase().includes(q) ||
      (t.wash_type || '').toLowerCase().includes(q)
    )
  })

  // Executive Metric Counts
  const totalVerifiedPiecesWashed = historyTasks.reduce((sum, t) => sum + (Number(t.completed_pieces || t.pieces_to_wash) || 0), 0)
  const uniqueArticlesCount = new Set(historyTasks.map(t => t.article_number)).size
  const activeAssignmentsCount = effectiveTasks.filter(t => t.status !== 'VERIFIED_COMPLETED' && t.status !== 'WORKER_COMPLETED').length
    ...
    <div className="space-y-5 sm:space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Waves className="w-3.5 h-3.5 text-[#3A3564]" />
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600 mt-1">
              Garment washing assignments completed by <strong>{displayWorkerName}</strong> and verified by the Head of Department
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        {/* Metric 1: Verified Pieces Washed */}
        <div className="bg-white p-4 rounded-2xl border border-black/10 shadow-2xs flex flex-col justify-between">
          ...
          <div className="mt-3">
            <div className="text-xs font-bold uppercase tracking-wider text-slate-900 font-mono">
              Verified Pieces Washed
            </div>
            <div className="text-2xl sm:text-3xl font-bold font-mono text-slate-900 font-[family-name:var(--font-heading)] mt-0.5">
              {totalVerifiedPiecesWashed.toLocaleString('en-IN')} <span className="text-xs font-normal text-slate-500">Pcs</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5 border-b border-black/10 bg-[#FAF7F0]/40 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder="Search by task #, article style, buyer, wash recipe, drum..."
          ...
          <div className="p-12 text-center space-y-2">
            ...
            <p className="text-xs text-slate-500 font-mono max-w-sm mx-auto">
              {searchQuery
                ...
                : 'Once the Head of Department clicks "Verify & Done" on your completed washing batches, they will appear in this history archive.'}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs min-w-[700px]">
              <thead className="bg-[#FAF7F0] text-[#3A3564] font-mono uppercase text-[11px] tracking-wider border-b border-black/10">
                <tr>
                  ...
                  <th className="py-3 px-4 font-bold">Drum / Machine Station</th>
                  <th className="py-3 px-4 font-bold">Wash Recipe</th>
                  <th className="py-3 px-4 font-bold text-right">Washed Pieces</th>
                ...
                {filteredHistory.map(task => (
                  <tr key={task.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-mono font-semibold text-slate-700">
                      {task.machine_station || 'Drum 01'}
                    </td>
                    <td className="py-3.5 px-4 font-mono">
                      <span className="px-2 py-0.5 rounded bg-[#FAF7F0] border border-black/10 text-[10px] font-bold text-[#3A3564]">
                        {task.wash_type || 'Silicon Soft Wash'}
                      </span>
                    </td>
                    <td className="py-3.5 px-4 font-mono font-bold text-right text-slate-900">
                      +{task.pieces_to_wash.toLocaleString('en-IN')} Pcs
                    </td>
