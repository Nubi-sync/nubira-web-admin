...
export default async function CentralStoreEmbroideryPage() {
  ...
  const [receiptsRes, issuesRes, allIssuesRes] = await Promise.all([
    fetchMaterialReceiptsByDivision('EMBROIDERY', companyFilter),
    fetchMaterialIssuesByDivision('EMBROIDERY', companyFilter),
    fetchMaterialIssuesByDivision(undefined, companyFilter),
  ])

  const initialReceipts = receiptsRes.data || []
  const initialIssues = (issuesRes.data || []).filter((i: any) => i.from_division === 'EMBROIDERY')
  const pendingIssuesForMe = (allIssuesRes.data || []).filter(
    (i: any) => i.to_division === 'EMBROIDERY' && i.status !== 'RECEIVED'
  )
