...
export function WorkerProfileClient({
  ...
  const rolesList: string[] = workerRecord?.roles || (workerRecord?.role ? [workerRecord.role] : ['EMBROIDERY_OPERATOR'])
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600 mt-1 font-mono">
              Floor ID: {workerRecord?.id || userId ? `OP-${(workerRecord?.id || userId).slice(-6)}` : 'OP-EMB-01'} • Nubira Creation Floor
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-black/10 shadow-2xs space-y-4">
          ...
          <p className="text-xs text-slate-500 font-mono">
            Operational roles assigned to this operator by the Embroidery Master &amp; Floor Supervisor
          </p>
          ...
          <div className="p-3.5 rounded-xl bg-slate-50 border border-black/5 text-xs text-slate-600 font-mono space-y-1 mt-4">
            ...
            <div>• Receive article embroidery piece quotas &amp; multi-head machine stations</div>
            <div>• Start live stitching clock and record machine runs</div>
