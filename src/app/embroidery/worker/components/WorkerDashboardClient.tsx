...
import { EmbroideryTaskAllocation, EmbroideryWorker } from '../../types/embroidery'
import {
...
import { saveEmbroideryTaskAllocationAction, fetchEmbroideryTaskAllocationsAction } from '../../actions'
...
export function WorkerDashboardClient({
  ...
  useEffect(() => {
    setUnreadCount(getUnreadNotificationCount(companyName, 'embroidery'))
  ...
  const handleManualSync = async () => {
    ...
    try {
      ...
      setTasks(merged)
      toast.success('Workstation updated with latest embroidery assignments.')
    } catch {
      reloadData()
    } finally {
      setIsSyncing(false)
  ...
  let displayWorkerName = userName || 'Embroidery Operator'
  const isGeneric = (n?: string) => !n || ['floor operator', 'embroidery operator', 'embroidery floor operator'].includes(n.trim().toLowerCase())
  ...
  const totalVerifiedPiecesEmbroidered = historyTasks.reduce((sum, t) => sum + (Number(t.completed_pieces || t.pieces_to_embroider) || 0), 0)
  const activePiecesTarget = currentTasks.reduce((sum, t) => sum + (Number(t.pieces_to_embroider) || 0), 0)
  ...
  // Worker Action 1: Start Embroidery (Starts countdown timer)
  const handleStartEmbroidery = async (taskId: string, taskRef: string, machineName?: string, allotedHours = 4) => {
    ...
    const updatedTasks = tasks.map(t => {
      if (t.id === taskId || t.task_ref === taskRef) {
        return {
          ...
          due_time: dueTime,
          table_number: machineName || t.table_number || 'Machine 01 (Tajima 20-Head)',
    ...
    updateEmbroideryTaskStatus(taskId, 'IN_PROGRESS', {
      task: existingTask,
      started_at: startedAt,
      due_time: dueTime,
      table_number: machineName || existingTask?.table_number || 'Machine 01 (Tajima 20-Head)'
    })
    ...
    toast.success(`Task #${taskRef} started! ${allotedHours} hr embroidery countdown timer is running on ${machineName || 'Machine 01'}.`)
  }
  ...
  const handleFinishWork = async (taskId: string, taskRef: string, pieces: number) => {
    ...
    updateEmbroideryTaskStatus(taskId, 'WORKER_COMPLETED', {
      task: existingTask,
      completed_pieces: pieces
    })
    ...
    if (taskObj) {
      ...
      broadcastFloorEvent({
        ...
        title: `Work Finished: ${taskObj.worker_name || 'Embroidery Operator'}`,
        message: `${taskObj.worker_name || 'Embroidery Operator'} completed ${pieces.toLocaleString('en-IN')} pcs of Article ${taskObj.article_number} (Lot ${taskRef}). Ready for verification.`,
    ...
    <div className="space-y-5 sm:space-y-6">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600 mt-1">
              Your assigned garment embroidery quotas, multi-head machine stations, and shift sign-off desk
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="bg-white p-4 rounded-2xl border border-black/10 shadow-2xs flex flex-col justify-between">
          ...
          <div className="mt-3">
            <div className="text-xs font-bold uppercase tracking-wider text-slate-900 font-mono">
              Verified Pieces Embroidered
            </div>
            <div className="text-2xl sm:text-3xl font-bold font-mono text-slate-900 font-[family-name:var(--font-heading)] mt-0.5">
              {totalVerifiedPiecesEmbroidered.toLocaleString('en-IN')} <span className="text-xs font-normal text-slate-500">Pcs</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-1">
          <div>
            <h2 className="text-lg font-bold text-slate-900 font-[family-name:var(--font-heading)]">
              Active Embroidery Tasks
            </h2>
            <p className="text-xs text-slate-500 font-mono">
              Execute multi-head machine runs, start hooping stations, and submit when finished
            </p>
          </div>
        </div>
        ...
        {currentTasks.length === 0 ? (
          <div className="bg-white p-12 rounded-3xl border border-black/10 shadow-2xs text-center space-y-2">
            ...
            <p className="text-xs text-slate-500 font-mono max-w-sm mx-auto">
              You have no pending embroidery allocations in your queue. New assignments from the Head of Department will appear here.
            </p>
          </div>
        ) : (
