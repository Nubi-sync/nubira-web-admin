...
import { saveEmbroideryWorker } from '../utils/embroideryFloorStorage'
import { registerEmbroideryWorkerAction } from '../actions'

interface AddWorkerModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess?: (worker: EmbroideryWorker) => void
}

const AVAILABLE_ROLES: { id: EmbroideryWorkerRole; label: string }[] = [
...
export function AddWorkerModal({ isOpen, onClose, onSuccess }: AddWorkerModalProps) {
  ...
  const [selectedRoles, setSelectedRoles] = useState<EmbroideryWorkerRole[]>(['EMBROIDERY_OPERATOR'])
  ...
  const toggleRole = (roleId: EmbroideryWorkerRole) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newWorker: EmbroideryWorker = {
        id: `ew-${Date.now()}`,
      ...
      saveEmbroideryWorker(newWorker)

      // Provision Auth User & database row via server action
      const res = await registerEmbroideryWorkerAction({
        worker_name: name.trim(),
        phone_number: phone.trim(),
      ...
      setPhone('')
      setPassword('')
      setSelectedRoles(['EMBROIDERY_OPERATOR'])
      onClose()
    } catch (err: any) {
