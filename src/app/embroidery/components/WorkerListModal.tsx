...
import { EmbroideryWorker, EmbroideryWorkerRole } from '../types/embroidery'
import { deleteEmbroideryWorker, saveEmbroideryWorker } from '../utils/embroideryFloorStorage'
import { ConfirmDialog } from '@/components/ui/ConfirmDialog'
import { deleteEmbroideryWorkerAction } from '../actions'
...
const ROLE_LABELS: Record<string, string> = {
  EMBROIDERY_OPERATOR: 'Multi-Head Machine Operator',
  HOOPING_SPECIALIST: 'Hooping & Framing Specialist',
  PUNCH_DIGITIZER: 'Punch Digitizer / Quality Inspector'
}
...
export function WorkerListModal({
  ...
  const [workerToDelete, setWorkerToDelete] = useState<EmbroideryWorker | null>(null)
  ...
  const filteredWorkers = workers.filter(w => {
    ...
    const matchesRole = 
      roleFilter === 'ALL' || 
      (w.roles && w.roles.includes(roleFilter as EmbroideryWorkerRole)) ||
  ...
  const handleConfirmDeleteWorker = async () => {
    ...
    try {
      ...
      deleteEmbroideryWorker(workerToDelete.id)
      if (workerToDelete.worker_name) deleteEmbroideryWorker(workerToDelete.worker_name)
      if (workerToDelete.phone_number) deleteEmbroideryWorker(workerToDelete.phone_number)

      if (onDeleteWorker) {
        await onDeleteWorker(workerToDelete.id, workerToDelete.phone_number)
      } else {
        await deleteEmbroideryWorkerAction(workerToDelete.id, workerToDelete.phone_number)
      }

      toast.success(`Worker "${workerToDelete.worker_name}" removed from embroidery roster.`)
      onWorkersUpdated()
    } catch (err) {
  ...
  const handleToggleStatus = (worker: EmbroideryWorker) => {
    const nextStatus: EmbroideryWorker['status'] = worker.status === 'ACTIVE' ? 'ON_LEAVE' : 'ACTIVE'
    saveEmbroideryWorker({ ...worker, status: nextStatus })
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-150 select-none">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-bold text-slate-900 font-[family-name:var(--font-heading)]">
                  Embroidery Floor Workers Roster
                </h2>
              ...
              <p className="text-xs text-slate-500 font-mono">
                Manage registered machine operators, hooping specialists, and portal credentials
              </p>
            </div>
          </div>
        ...
        <div className="p-4 border-b border-black/10 bg-slate-50/70 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
            {[
              ...
              { id: 'EMBROIDERY_OPERATOR', label: 'Machine Operator' },
              { id: 'HOOPING_SPECIALIST', label: 'Hooping Spec' },
              { id: 'PUNCH_DIGITIZER', label: 'Digitizer' }
            ].map(r => (
              <button
                key={r.id}
                type="button"
        ...
        <div className="p-6 overflow-y-auto space-y-3">
          {filteredWorkers.length === 0 ? (
            <div className="py-12 text-center text-slate-400 space-y-2">
              ...
              <p className="text-sm font-semibold">No registered embroidery workers found.</p>
              <button
                type="button"
                onClick={() => {
                  onClose()
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {filteredWorkers.map(worker => {
                ...
                const displayRoles = worker.roles && worker.roles.length > 0
                  ...
                  : (worker.role ? [worker.role] : ['Multi-Head Machine Operator'])
      ...
      <ConfirmDialog
        ...
        title="Remove Embroidery Worker"
        description={`Are you sure you want to remove "${workerToDelete?.worker_name}" from the Embroidery Floor roster? They will no longer be available for shift task allocations.`}
            ...
            <div className="flex items-center justify-between border-t border-black/5 pt-2">
              ...
              <span className="font-bold text-slate-800">
                {(workerToDelete.roles && workerToDelete.roles.length > 0)
                  ...
                  : (workerToDelete.role || 'Multi-Head Machine Operator')}
              </span>
            </div>
          </div>
        )}
