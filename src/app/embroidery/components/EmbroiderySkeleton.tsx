...
export function SkeletonMachineGrid({ count = 4 }: { count?: number }) {
  return (
...
export function EmbroideryPageSkeleton({
  variant = 'dashboard'
}: {
  variant?:
    | 'dashboard'
    ...
    <div className="p-4 sm:p-6 md:p-8 space-y-6 max-w-7xl mx-auto select-none">
      ...
      {variant === 'dashboard' && (
        <>
          ...
          <SkeletonMachineGrid count={4} />
        </>
      )}

      {variant === 'punch-library' && (
        <>
          <SkeletonFilterBar />
