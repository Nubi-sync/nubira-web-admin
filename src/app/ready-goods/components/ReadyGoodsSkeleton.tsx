...
export function SkeletonCartonGrid({ count = 6 }: { count?: number }) {
  return (
...
export function ReadyGoodsPageSkeleton({
  variant = 'dashboard'
}: {
  variant?:
    | 'dashboard'
    ...
    <div className="p-4 sm:p-6 md:p-8 space-y-6 max-w-7xl mx-auto select-none">
      ...
      {variant === 'dashboard' && (
        <>
          ...
          <SkeletonCartonGrid count={6} />
        </>
      )}

      {variant === 'tagging-polybag' && (
        <>
          <SkeletonFilterBar />
